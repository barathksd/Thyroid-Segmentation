...
def load_model():
    ...
    model.compile(loss=cnndemo.ioc_loss,
                  optimizer=opt,
                  metrics=['accuracy'])
    return model
...
img_train = np.concatenate([aug_x_train, aug_x_extra_train])[train_order].reshape(60000+I,28,28,1)
label_train = np.concatenate([y_train, y_extra_train])[train_order].reshape(60000+I)

test_order = np.random.permutation([j for j in range(10000+J)])
img_test = np.concatenate([aug_x_test, aug_x_extra_test])[test_order].reshape(10000+J,28,28,1)
label_test = np.concatenate([y_test, y_extra_test])[test_order].reshape(10000+J)
